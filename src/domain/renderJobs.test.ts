...
import { DEFAULT_CHAPTER_POLICY, DEFAULT_IMAGE_SEQUENCE_PROFILE } from './deliveryProduct'
...
const job: RenderJob = {id:'a',name:'A',sequenceId:'s',binding:'local',revision:'a'.repeat(64),range:{startFrame:4,endFrame:5},profile:DEFAULT_EXPORT_PROFILE,chapters:DEFAULT_CHAPTER_POLICY,status:'queued',delivery:'none',attempts:0,message:''}
describe('persistent render facts',()=>{
  ...
  it('accepts classic jobs without chapters and tagged image-sequence products',()=>{
    const {chapters: _chapters, ...classic} = job
    expect(parseRenderJob(classic).chapters).toEqual(DEFAULT_CHAPTER_POLICY)
    const sequence = parseRenderJob({...job, profile: DEFAULT_IMAGE_SEQUENCE_PROFILE, chapters: { mode: 'sidecar' }})
    expect(sequence.profile).toMatchObject({ kind: 'image-sequence', format: 'png' })
    expect(sequence.chapters.mode).toBe('sidecar')
  })
