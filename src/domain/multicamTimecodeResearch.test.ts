...
import { alignResearchTimecodes, parseResearchTimecode, type ResearchTimecodeEvidence } from './multicamTimecodeResearch'

const F30 = { num: 30, den: 1 }
const evidence = (patch: Partial<ResearchTimecodeEvidence> = {}): ResearchTimecodeEvidence => ({
  format: 'normalized-timecode-research-v1', label: '01:02:03:04', rate: F30,
...
describe('Issue 194 normalized timecode proof (no metadata adapter)', () => {
  test('counts a declared non-drop label without interpreting wall-clock seconds', () => {
    expect(parseResearchTimecode(evidence())).toMatchObject({ state: 'valid', frameCount: 111_694 })
    expect(parseResearchTimecode(evidence({ rate: { num: 30_000, den: 1_001 } }))).toMatchObject({
  ...
  test('subtracts source origins exactly with the same sign as the audio proposal', () => {
    ...
    expect(alignResearchTimecodes(ref, later, F30)).toEqual({ state: 'aligned', offsetFrames: 65 })
    expect(alignResearchTimecodes(later, ref, F30)).toEqual({ state: 'aligned', offsetFrames: -65 })
    const ntsc = { num: 30_000, den: 1_001 }
    expect(alignResearchTimecodes(
  ...
  test.each([
    ...
    expect(parseResearchTimecode(value).state).toBe('unavailable')
    expect(alignResearchTimecodes(evidence(), value, F30).state).toBe('unavailable')
  })
  ...
  test.each([
    ...
    expect(parseResearchTimecode(evidence({ label }))).toEqual({ state: 'unavailable', reason: 'invalid-label' })
  })

  test('rejects mismatched clocks, rates, and implicit frame-rate conversion', () => {
    expect(alignResearchTimecodes(evidence(), evidence({ clockDomain: 'unrelated' }), F30)).toMatchObject({ reason: 'different-clocks' })
    expect(alignResearchTimecodes(evidence(), evidence({ rate: { num: 25, den: 1 } }), F30)).toMatchObject({ reason: 'different-rates' })
    expect(alignResearchTimecodes(evidence(), evidence(), { num: 60, den: 1 })).toMatchObject({ reason: 'different-rates' })
  })

  test('copies normalized evidence and does not invent a midnight wrap', () => {
    const source = evidence({ rate: { ...F30 } })
    const parsed = parseResearchTimecode(source)
