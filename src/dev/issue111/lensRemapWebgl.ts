...
import { RENDER_SURFACE_BYTES_PER_PIXEL } from '../../domain/renderSurfaceBudget'
...
export class WebGl2LensRemapBackend {
  ...
  private assertUsable(): void {
    ...
    if (this.gl.isContextLost()) throw new Error('Lens-remap WebGL2 context was lost')
  }
  ...
  render(
    ...
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA8, width, height, 0, gl.RGBA, gl.UNSIGNED_BYTE, input)
    ...
    if (error !== gl.NO_ERROR) throw new Error(`Lens-remap WebGL2 render failed with error ${error}`)
