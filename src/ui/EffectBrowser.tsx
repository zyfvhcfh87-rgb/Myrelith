...
function BrowserDialog({ clipId, saving, onClose }: { clipId: string; saving: boolean; onClose(): void }) {
  ...
  const [capture] = useState<{ save: PresetSaveSession | null; error: string | null }>(() => {
    ...
    try { return { save: openPresetSave(clipId), error: null } }
  ...
  const apply = (effects: Parameters<typeof applyEffectTemplate>[1]) => {
    const failure = applyEffectTemplate(session, effects, mode)
  ...
  return <dialog ref={ref} className="text-overlay-dialog attribute-dialog effect-browser" aria-labelledby={titleId}
    ...
    <div className="text-overlay-dialog-card">
      ...
      <p>Target: clip effects on {session.targetNames.length} selected clip{session.targetNames.length === 1 ? '' : 's'}: {session.targetNames.join(', ')}.</p>
      <label className="text-overlay-dialog-field">Apply stack<select aria-label="Apply stack" value={mode} onChange={(event) => setMode(event.target.value as 'append' | 'replace')}>
        <option value="append">Append effects</option><option value="replace">Replace entire stack and its keys</option>
      </select></label>
      ...
      <ul className="effect-browser-list" aria-label="Matching effects">
        {(filter === 'all' || filter === 'builtins') && builtins.map((entry) => <li key={entry.effect.type}>
          ...
          {effectTemplatePreview([entry.effect], capabilities).map((detail) => <p key={detail}>{detail}</p>)}
          <button type="button" onClick={() => apply([entry.effect])}>Apply {entry.label}</button>
        </li>)}
        {(filter === 'all' || filter === 'presets') && presets.map((preset) => <li key={preset.id}>
          ...
          {effectTemplatePreview(preset.effects, capabilities).map((detail) => <p key={detail}>{detail}</p>)}
          {preset.effects.filter((effect) => effect.type.startsWith('plugin:')).map((effect) => {
          ...
          <div className="inspector-effect-actions">
            <button type="button" onClick={() => apply(preset.effects)}>Apply preset {preset.name}</button>
        ...
        {(filter === 'all' || filter === 'plugins') && contributions.map((entry) => <li key={entry.effectType}>
          ...
          {session.targetIds.length !== 1 && <p>Select one clip to add a plugin contribution, then copy its stack for batch reuse.</p>}
          <button type="button" disabled={!entry.selectAction.available || entry.selectAction.pending || !!entry.selectAction.disabledReason || session.targetIds.length !== 1 || !editor?.coherent || editor.catalogGeneration === null || mode !== 'append'} onClick={() => {
...
export default function EffectBrowser({ clipId, hasEffects }: { clipId: string; hasEffects: boolean }) {
  ...
  return <>
    ...
    {mode && <BrowserDialog clipId={clipId} saving={mode === 'save'} onClose={() => setMode(null)} />}
  </>
}
