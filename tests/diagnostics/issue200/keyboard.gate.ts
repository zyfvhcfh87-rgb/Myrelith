...
async function controls(page: Page) {
  ...
  const options = await fallback.evaluate((element) => {
    if (!(element instanceof HTMLSelectElement)) throw new Error('Expected the native fallback select')
    return Array.from(element.options, (item) => ({ value: item.value, text: item.text, disabled: item.disabled }))
  })
  run().events.push({ kind: 'fallback-options', options })
  expect(options).toEqual(['', 'sans-serif', 'serif', 'monospace', 'cursive', 'fantasy', 'system-ui'].map((value) => ({ value, text: value || 'No fallback', disabled: false })))
  ...
  await expect.poll(() => fallback.evaluate((element) => element.matches(':open'))).toBe(false)
  ...
  unchanged(missing, await state(page, 'serif-navigation'))
  await page.keyboard.press('Enter')
  await expect.poll(() => fallback.evaluate((element) => element.matches(':open'))).toBe(false)
  await expect(fallback).toHaveValue('serif')
