...
spec = importlib.util.spec_from_file_location('keyboard_runner', Path(__file__).resolve().parents[3] / 'docs/evidence/issue200/run-keyboard-browser.py')
...
class RunnerTests(unittest.TestCase):
    def test_manifest_includes_trace_bytes_after_exit_and_excludes_itself(self):
        import hashlib
        with tempfile.TemporaryDirectory(prefix='issue200-keyboard-manifest-unit-', dir='/private/tmp') as folder:
            root = Path(folder)
            (root / 'results').mkdir()
            payload = b'retained-success-trace-bytes\x00\xff'
            (root / 'results' / 'trace.zip').write_bytes(payload)
            with patch.dict(runner.os.environ, ISSUE200_KEYBOARD_ARTIFACTS=folder):
    ...
    def test_main_times_out_and_kills_without_stdout_eof_or_term_exit(self):
        ...
        def send(pid, sig):
            ...
            if sig == signal.SIGKILL:
                child.returncode = -9
        with tempfile.TemporaryDirectory(prefix='issue200-keyboard-source-unit-', dir='/private/tmp') as temporary:
            directory = Path(temporary)
            manifest = directory / 'manifest.json'
            manifest.write_text('{}')
            artifacts = directory / 'artifacts'
            with patch.dict(runner.os.environ, ISSUE200_KEYBOARD_ARTIFACTS=str(artifacts), ISSUE200_KEYBOARD_MANIFEST=str(manifest)), \
