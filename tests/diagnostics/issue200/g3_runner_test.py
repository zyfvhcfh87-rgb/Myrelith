...
spec = importlib.util.spec_from_file_location('g3_runner', Path(__file__).resolve().parents[3] / 'docs/evidence/issue200/run-g3-browser.py')
    ...
    def test_main_times_out_and_kills_without_stdout_eof_or_term_exit(self):
        ...
        def send(pid, sig):
            ...
            if sig == signal.SIGKILL:
                child.returncode = -9
        with tempfile.TemporaryDirectory(prefix='issue200-g3-source-unit-', dir='/private/tmp') as temporary:
            directory = Path(temporary)
            manifest = directory / 'manifest.json'
            manifest.write_text('{}')
            artifacts = directory / 'artifacts'
            with patch.dict(runner.os.environ, ISSUE200_G3_ARTIFACTS=str(artifacts), ISSUE200_G3_MANIFEST=str(manifest)), \
