...
test('grading through nested scopes matches explicit child composition with transitions, captions, adjustments, masks and lens correction', async ({ page }, info) => {
  await page.goto('/')
  const result = await page.evaluate(async () => {
    ...
    const r = '/src/pipeline/render.ts', e = '/src/domain/effectStack.ts'
    ...
    const { createColorAdjustEffect, createMaskEffect, resolvePostCompositeEffectStack } = await import(e)
    ...
    const gradingPath = '/src/pipeline/colorGradingRuntime.ts', lutPath = '/src/domain/colorLut.ts', curvePath = '/src/domain/colorCurves.ts', wheelPath = '/src/domain/colorWheels.ts'
    const { ColorGradingRuntime } = await import(gradingPath), { parseCube, portableColorLut } = await import(lutPath)
    const { DEFAULT_COLOR_CURVES } = await import(curvePath), { DEFAULT_COLOR_WHEELS } = await import(wheelPath)
    const runtime = new ColorGradingRuntime(); runtime.setCatalog([portableColorLut('table', 'Curve', parseCube('LUT_1D_SIZE 2\n0.04 0.08 0.12\n0.9 0.95 1'))])
    const grading = { runtime, context: runtime.context, policy: 'fail', check: () => {} }
    const grade = (id: string, kind: 'lut' | 'curve' | 'wheel') => ({ id, version: 1, enabled: true, type: kind === 'lut' ? 'builtin.cube-lut' : kind === 'curve' ? 'builtin.rgb-curves' : 'builtin.lift-gamma-gain', params: kind === 'lut' ? { lutId: 'table', strength: 0.8 } : kind === 'curve' ? { ...DEFAULT_COLOR_CURVES, red: '[[0,0],[0.4,0.6],[1,1]]' } : { ...DEFAULT_COLOR_WHEELS, gainB: 0.8, liftR: 0.1 } })
    ...
    const post = async (target: ReturnType<typeof surface>, effects: unknown[]) => {
      const data = target.ctx.getImageData(0, 0, width, height)
      await runtime.apply(data.data, resolvePostCompositeEffectStack(effects, true, runtime.context).pixelEffects, { surfaceWidth: width, surfaceHeight: height, projectWidth: width, projectHeight: height })
    ...
    const render = async (documents: Array<{ id: string }>, root: string) => {
      ...
      await compositeFrame(doc, plan, output.ctx, { getFrame: async (id: string) => images.get(id) ?? null }, { get: () => ({ leg, group }) }, undefined, lensProvider, undefined, grading)
    ...
    try {
      ...
      const fade = { ...track('child-fade', [from, to]), transitions: [{ id: 'fade', type: 'crossfade', fromClipId: from.id, toClipId: to.id, durationFrames: 1, audio: { enabled: false, curve: 'equal-power' } }], videoEffects: [grade('child-track', 'lut')] }
      child.tracks = [track('child-lower', [clip('child-base', 'plate')]), fade, { ...track('adjustment'), adjustments: [{ kind: 'adjustment', id: 'adjustment', name: 'Adjustment', timelineRange: { startFrame: 0, durationFrames: 60 }, enabled: true, opacity: 0.3, animation: { tracks: [], effectTracks: [] }, effects: [grade('adjustment-color', 'curve')] }], videoEffects: [color('must-not-run-on-adjustment', 2)] }]
      child.captionTracks = [caption('child-captions', 'Child')]
      child.masterVideoEffects = [grade('child-master', 'wheel')]
      ...
      overlay.effects = [grade('overlay-lut', 'lut'), mask, grade('overlay-curve', 'curve')]
      ...
      root.tracks = [track('root-lower', [clip('root-base', 'blue')]), parentTrack, { ...track('root-overlay', [overlay]), videoEffects: [grade('overlay-track', 'wheel')] }]
      root.captionTracks = [caption('root-captions', 'Parent')]; root.masterVideoEffects = [grade('root-master', 'curve')]
      ...
      await post(isolated.output, child.masterVideoEffects); await post(isolated.output, parentTrack.videoEffects)
      ...
      await post(reference.output, root.masterVideoEffects)
      ...
      runtime.dispose(); backend?.dispose()
  ...
  const evidencePath = info.outputPath('grading-nested-render.json')
