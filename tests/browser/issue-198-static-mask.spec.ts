...
test('held path keys preserve fallback, change Program pixels at the key and support direct edits and undo', async ({ page }) => {
  ...
  await field.fill(path); await field.blur()
  await expect(page.getByText('2 held path keys.', { exact: false })).toBeVisible()
  const keyed = await snapshot(page), clip = keyed.project.sequences[0].tracks[0].clips[0]
  expect(clip.effects[0].params.path).toBe(fallback)
  expect(clip.animation.effectPathTracks[0].keyframes.map((key: { frame: number; sourceTimeTicks: number }) => [key.frame, key.sourceTimeTicks])).toEqual([[0, 0], [15, 15_000_000]])
  expect(keyed.past).toBe(initial.past + 2)
  await expect.poll(() => pixel(page, 0.6, 0.6)).toEqual([0, 0, 0, 255])
  await seek(14)
  await expect.poll(() => pixel(page, 0.6, 0.6)).toEqual([64, 128, 192, 255])
  await seek(15)
  ...
  expect(edited.past).toBe(keyed.past + 1)
  expect(edited.project.sequences[0].tracks[0].clips[0].effects[0].params.path).toBe(fallback)
  expect(edited.project.sequences[0].tracks[0].clips[0].animation.effectPathTracks[0].keyframes).toHaveLength(2)
  await page.evaluate(async () => { const d = '/src/state/documentStore.ts'; (await import(d)).useDocumentStore.getState().undo() })
  expect((await snapshot(page)).project).toEqual(keyed.project)
  await page.getByRole('button', { name: 'Previous path key', exact: true }).click()
  await expect(field).toHaveValue(String(fallback))
  await page.getByRole('button', { name: 'Next path key', exact: true }).click()
  await expect(field).toHaveValue(path)
  await page.screenshot({ path: '.tmp/issue198-path-key.png' })
  await page.getByRole('button', { name: 'Clear path keys', exact: true }).click()
  await expect.poll(() => pixel(page, 0.6, 0.6)).toEqual([64, 128, 192, 255])
  await page.evaluate(async () => { const d = '/src/state/documentStore.ts'; (await import(d)).useDocumentStore.getState().undo() })
  expect((await snapshot(page)).project).toEqual(keyed.project)
  await expect.poll(() => pixel(page, 0.6, 0.6)).toEqual([0, 0, 0, 255])
