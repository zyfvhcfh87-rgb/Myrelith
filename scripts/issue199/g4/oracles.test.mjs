...
function passing() { return { size: 2000, width: 1280, height: 720, duration: 1, videoDuration: 1, videoCodec: 'vp9', audioCodec: 'opus', videoPackets: { packetCount: 30, averagePacketRate: 30 },
...
for (const fault of ['missing glyph', 'stationary crawl', 'wrong gain', 'wrong pan', 'missing PCM', 'wrong fps', 'RGB mismatch']) test(`mixed oracle rejects ${fault}`, () => {
  const r = passing()
  ...
  if (fault === 'wrong fps') r.videoPackets.averagePacketRate = 29.97
