...
const profile = path.join(labRoot, 'profile-02')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-diagnostic-runtime-source',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-02.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-02.jsonl')
...
try {
  ...
  const output = path.join(labRoot, 'runtime-results-02.json')
