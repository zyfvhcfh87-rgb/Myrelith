...
const manifest = JSON.parse(read('docs/evidence/issue201/whispercpp-diagnostic-runtime-source/manifest.json'))
...
test('original bounded native audio preparation is byte-identical in the separate worker', () => {
  ...
  assert.equal(extract(read('scripts/issue201/whispercpp-diagnostic-run/model-worker.mjs')), extract(read('scripts/issue201/model-worker.mjs')))
})
...
async function workerHarness({ corruptGlue = false, coreError = false, nativeBoundary = null } = {}) {
  const events = [], calls = [], imports = [], fetched = [], nativeCalls = []
  ...
  const context = vm.createContext({ crypto: webcrypto, performance, TextEncoder, TextDecoder, ArrayBuffer, Uint8Array, Float32Array, Blob,
    WebAssembly: undefined, self: { postMessage: value => events.push(value), close: () => { closed = true } },
    caches: { open: async () => ({ match: async () => ({ headers, arrayBuffer: async () => nativeBoundary ? fixtureBuffer(readFileSync(new URL(manifest.model.files[0].localPath, root))) : new ArrayBuffer(16) }) }) },
  ...
  const modules = {
    ...
    './observe-module.mjs': await import('../whispercpp-diagnostic/observe-module.mjs'),
  ...
  const module = new vm.SourceTextModule(read('scripts/issue201/whispercpp-diagnostic-run/model-worker.mjs'), { context,
    identifier: 'file:///inert/model-worker.mjs', importModuleDynamically: async specifier => {
      ...
      return synthetic(context, { default: options => {
        factoryCalls++
        if (!nativeBoundary) throw new Error('The source probe must not call a factory')
        function abortAt(stage) {
          options.printErr(`inert native stderr at ${stage}`); options.onAbort('')
          throw new Error(`inert abort at ${stage}`)
        }
        if (nativeBoundary === 'factory-failure') abortAt('factory')
        let owned = 0
        const fake = { HEAPU8: new Uint8Array(64 * 1024 * 1024),
          _speech_model_alloc(bytes) { nativeCalls.push({ method: '_speech_model_alloc', bytes });
            if (nativeBoundary === 'alloc-failure') abortAt('_speech_model_alloc'); return 4 },
          _speech_load() { nativeCalls.push({ method: '_speech_load' });
            if (nativeBoundary === 'load-failure') abortAt('_speech_load'); owned = 1; return 0 },
          _speech_close() { nativeCalls.push({ method: '_speech_close' }); owned = 0; return 0 },
          _speech_owned() { return owned },
        }
        return fake
      } })
    } })
  await module.link(async specifier => {
    if (nativeBoundary && ['./candidate.mjs', './worker-protocol.mjs'].includes(specifier)) {
      const file = specifier === './candidate.mjs' ? 'whispercpp-diagnostic-run/candidate.mjs' : 'whispercpp/worker-protocol.mjs'
      const actual = new vm.SourceTextModule(read('scripts/issue201/' + file), { context })
      await actual.link(async nested => {
        assert.equal(nested, './model-format.mjs')
        const format = new vm.SourceTextModule(read('scripts/issue201/whispercpp/model-format.mjs'), { context })
        await format.link(() => { throw new Error('Unexpected format import') }); return format
      })
      return actual
    }
    assert.ok(modules[specifier], specifier); return synthetic(context, modules[specifier])
  })
  ...
  return { events, calls, imports, fetched, nativeCalls, context, send, initialize, closed: () => closed, factoryCalls: () => factoryCalls }
}
...
test('all original acceptance case bodies are unchanged except the reviewed browser reopen lifecycle', () => {
  ...
  const runner = read('scripts/issue201/whispercpp-diagnostic-run/run-lab.mjs')
...
test('diagnostic metadata changes cache identity while exact native artifacts/model/limits remain unchanged', async () => {
  const original = JSON.parse(read('docs/evidence/issue201/whispercpp-runtime-source/manifest.json'))
  const { diagnostics, ...runtime } = manifest.runtime
  assert.deepEqual(runtime, original.runtime)
  assert.deepEqual(manifest.model, original.model)
  assert.notEqual(modelCachePayload(manifest), modelCachePayload(original))
  const observer = readFileSync(new URL(diagnostics.sourcePath, root))
  assert.equal(observer.length, diagnostics.bytes); assert.equal(hash(observer), diagnostics.sha256)
})
for (const [nativeBoundary, stage, expectedCalls] of [
  ['factory-failure', 'factory', []],
  ['alloc-failure', '_speech_model_alloc', ['_speech_model_alloc', '_speech_close']],
  ['load-failure', '_speech_load', ['_speech_model_alloc', '_speech_load', '_speech_close']],
]) {
  test(`actual wired worker and frozen protocol publish diagnostics before ${nativeBoundary} error with no repeated native calls`, async () => {
    const worker = await workerHarness({ nativeBoundary }); await worker.initialize()
    const index = worker.events.findIndex(event => event.type === 'native-diagnostics')
    const error = worker.events.findIndex(event => event.type === 'error')
    assert.ok(index >= 0 && error > index)
    const detail = worker.events[index].detail
    assert.ok(detail.records.some(row => row.kind === 'stderr' && row.phase === stage && row.text.includes('inert native stderr')), JSON.stringify(worker.events))
    assert.ok(detail.records.some(row => row.kind === 'exception' && row.phase === stage))
    assert.equal(worker.events[error].code, 'initialization-failed')
    assert.match(worker.events[error].message, new RegExp('inert abort at ' + stage))
    assert.equal(worker.events.some(event => event.type === 'ready'), false)
    assert.equal(worker.factoryCalls(), 1)
    assert.deepEqual(worker.nativeCalls.map(call => call.method), expectedCalls)
  })
}
test('actual wired successful load preserves the ready result and invokes allocation/load exactly once', async () => {
  const worker = await workerHarness({ nativeBoundary: 'success' }); await worker.initialize()
  const diagnostic = worker.events.findIndex(event => event.type === 'native-diagnostics')
  const ready = worker.events.findIndex(event => event.type === 'ready')
  assert.ok(diagnostic >= 0 && ready > diagnostic, JSON.stringify(worker.events))
  assert.equal(worker.events[ready].heapBytes, 67108864)
  assert.equal(worker.events[ready].ledger.modelOwners, 1)
  assert.equal(worker.factoryCalls(), 1)
  assert.deepEqual(worker.nativeCalls.map(call => call.method), ['_speech_model_alloc', '_speech_load'])
  assert.equal(worker.nativeCalls[0].bytes, 43537433)
  await worker.send({ type: 'dispose' })
  assert.equal(worker.events.find(event => event.type === 'disposed').cooperativeZero, true)
})
