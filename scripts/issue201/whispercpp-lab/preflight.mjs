...
import { inspectGlue } from '../whispercpp-review/inspect-glue.mjs'

const hash = bytes => createHash('sha256').update(bytes).digest('hex')
export async function prepareRuntimeInputs(root, { verifyCheckpoint = true } = {}) {
  const evidencePath = 'docs/evidence/issue201/whispercpp-runtime-source'
  ...
  const modules = {
    'lab-client.mjs': 'whispercpp-lab/lab-client.mjs', 'model-worker.mjs': 'whispercpp-lab/model-worker.mjs',
    'candidate.mjs': 'whispercpp-lab/candidate.mjs', 'lab-contract.mjs': 'lab-contract.mjs',
  ...
  for (const [url, file] of Object.entries(modules)) {
    ...
    assets.set('/' + url, { bytes, contentType: 'text/javascript', immutable: true, sha256: hash(bytes) })
  }
