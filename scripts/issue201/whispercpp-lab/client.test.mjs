...
const manifest = JSON.parse(source('docs/evidence/issue201/whispercpp-runtime-source/manifest.json'))
...
async function controller() {
  ...
  const candidate = new vm.SourceTextModule(source('scripts/issue201/whispercpp-lab/candidate.mjs'), { context })
  await candidate.link(() => { throw new Error('Unexpected candidate import') })
  const client = new vm.SourceTextModule(source('scripts/issue201/whispercpp-lab/lab-client.mjs'), { context })
