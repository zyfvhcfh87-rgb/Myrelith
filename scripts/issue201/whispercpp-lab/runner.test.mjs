...
import * as monitor from './resident-monitor.mjs'
import * as hostLimits from './host-limits.mjs'

const root = new URL('../../../', import.meta.url)
const source = readFileSync(new URL('scripts/issue201/whispercpp-lab/run-lab.mjs', root), 'utf8')
const manifestBytes = readFileSync(new URL('docs/evidence/issue201/whispercpp-runtime-source/manifest.json', root))
...
async function runInert({ allowed = true, churn = false, profileExists = false, stall = null, writes = new Map() } = {}) {
  ...
  const profile = '/repo/.tmp/issue201-whispercpp-lab/profile-01'
  ...
  const modules = {
    ...
    './resident-monitor.mjs': monitor, './host-limits.mjs': stall === 'overall-work'
  ...
  const module = new vm.SourceTextModule(source, { context, identifier: 'file:///repo/scripts/issue201/whispercpp-lab/run-lab.mjs',
    initializeImportMeta: meta => { meta.url = 'file:///repo/scripts/issue201/whispercpp-lab/run-lab.mjs' } })
  ...
  return { error, writes, stdout, launches, closes, removes, kills, alive, requests, exitCode: processPort.exitCode,
    result: writes.has('/repo/.tmp/issue201-whispercpp-lab/runtime-results-01.json')
      ? JSON.parse(writes.get('/repo/.tmp/issue201-whispercpp-lab/runtime-results-01.json')) : null }
}
...
test('the attempt marker prevents a second run after an earlier failure', async () => {
  ...
  assert.equal(again.writes.get('/repo/.tmp/issue201-whispercpp-lab/runtime-results-01.json'),
    first.writes.get('/repo/.tmp/issue201-whispercpp-lab/runtime-results-01.json'))
})
for (const stall of ['page-operation', 'setup-cdp', 'browser-close', 'server-close']) {
  test(`actual runner reaches final evidence and bounded release when ${stall} never settles`, async () => {
    ...
    assert.equal(run.exitCode, 1)
    const partial = run.writes.get('/repo/.tmp/issue201-whispercpp-lab/runtime-partial-01.jsonl').trim().split('\n').map(JSON.parse)
