...
import { HOST_LIMITS, caseDeadlineMs } from './host-limits.mjs'
...
const profile = path.join(labRoot, 'profile-01')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-runtime-source',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-01.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-01.jsonl')
...
try {
  ...
  const output = path.join(labRoot, 'runtime-results-01.json')
