...
const profile = path.join(labRoot, 'profile-03')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-reservation-runtime',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-03.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-03.jsonl')
...
try {
  ...
  const output = path.join(labRoot, 'runtime-results-03.json')
