...
let coreCleanupAcknowledged = true
let inputOwner = null
let phase = 'created'
...
const cancelled = () => { if (cancelRequested) throw new Error('Speech cancelled') }
async function cancellationPoint() {
  // Yield to queued control messages, including after synchronous native work.
  // This is a scheduling boundary, not a guessed memory reclamation delay.
...
async function callCore(kind, payload = {}) {
  ...
  if (result.kind === 'error') {
    coreCleanupAcknowledged = result.cooperativeZero === true
    if (coreCleanupAcknowledged) ledger.modelOwners = 0
    core = null // The core protocol has ended and cannot accept a second close.
    throw new Error(`Core ${result.code}; cooperativeZero=${result.cooperativeZero}`)
  }
...
async function disposeOwned() {
  let audioClosed = true
  try {
    inputOwner?.dispose()
    inputOwner = null
    ledger.inputOwners = 0
  } catch { audioClosed = false }
  try {
    if (core) coreCleanupAcknowledged = (await callCore('close')).cooperativeZero === true
  } catch { /* An unacknowledged native close keeps admission unavailable. */ }
  core = null
  if (coreCleanupAcknowledged) ledger.modelOwners = 0
  disposed = true
  return audioClosed && coreCleanupAcknowledged && ledger.inputOwners === 0
    && ledger.modelOwners === 0 && ledger.sampleOwners === 0 && ledger.pcmBytes === 0
...
async function transcribe(message) {
  ...
  try {
    ...
    for (let start = 0; start < message.seconds; start += 25) {
      ...
      await cancellationPoint()
      let result = { timing: 'model', text: '', chunks: [] }
      if (output?.timing === 'unavailable') {
        if (output.reason !== 'timestamp-coverage' || typeof output.text !== 'string'
          || !output.text.trim() || output.text.length > 20_000 || 'segments' in output) throw new Error('Malformed untimed review output')
        result = { timing: 'unavailable', reason: output.reason, text: output.text, chunks: [] }
      } else if (output) {
        if (output.timing !== 'model' || !Array.isArray(output.segments)) throw new Error('Malformed timed review output')
        const timed = { text: output.segments.map(segment => segment.text).join(''),
          chunks: output.segments.map(segment => ({ text: segment.text,
            timestamp: [segment.fromCentiseconds / 100, segment.toCentiseconds / 100] })) }
        const chunks = speechSegments(timed, end - start)
        if (chunks.some(chunk => !chunk.timed)) throw new Error('Timed review output exceeds source coverage')
        result = { timing: 'model', text: timed.text, chunks }
      }
      ...
      // start/end describe the decoded source window, never untimed cue timing.
      windows.push({ start, end, elapsedMs: elapsed, ...result, digitalSilenceSkipped: zero, generatedTokens, heapBytes: output?.heapBytes ?? lastHeapBytes })
      ledger.windows++
      audio = null
      updatePcm(0)
...
self.onmessage = async ({ data }) => {
  if (disposed) return
  const control = data.type === 'cancel' || data.type === 'dispose'
  if (control) {
    cancelRequested = true
    if (messageBusy) return // The active operation closes at its next boundary.
  }
  ...
  try {
    if (data.type === 'initialize') await initialize(data)
      ...
      const cooperativeZero = await disposeOwned()
      post('disposed', { ledger: { ...ledger }, cooperativeZero })
      self.close()
    } else throw new Error('Unknown laboratory message')
  } catch (error) {
    const cooperativeZero = await disposeOwned()
    if (cancelRequested) post('disposed', { ledger: { ...ledger }, cooperativeZero })
    else post('error', { code: data.type === 'initialize' ? 'initialization-failed'
      : data.type === 'transcribe' ? 'transcription-failed' : 'disposal-failed', phase,
      message: error instanceof Error ? error.message : String(error), ledger: { ...ledger }, cooperativeZero })
    self.close()
