...
const profile = path.join(labRoot, 'profile-08')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-untimed-runtime.json',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-08.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-08.jsonl')
...
try {
  ...
  await check('offline-page-reload', async () => {
    ...
    try {
      ...
      const review = assessReviewWindows(result.windows, manifest.fixtures.find(fixture => fixture.name === 'english').durationSeconds)
      expect(review.untimedWindows === 0 && review.timedCues > 0, 'Offline fixture lost its validated timed output')
      return { result, review, cleanup: await page.evaluate(() => lab.cancel('offline-reload-complete')) }
    } finally {
  ...
  await check('offline-persistent-browser-reopen', async () => {
    ...
    try {
      ...
      const review = assessReviewWindows(result.windows, manifest.fixtures.find(fixture => fixture.name === 'english').durationSeconds)
      expect(review.untimedWindows === 0 && review.timedCues > 0, 'Offline fixture lost its validated timed output')
      return { result, review, cleanup: await page.evaluate(() => lab.cancel('offline-reopen-complete')) }
    } finally {
      await context.setOffline(false)
      if (!await page.evaluate(() => Boolean(globalThis.lab)).catch(() => false)) { await page.goto('http://127.0.0.1:5201/'); await page.waitForFunction(() => Boolean(globalThis.lab)) }
    }
  ...
  const output = path.join(labRoot, 'runtime-results-08.json')
