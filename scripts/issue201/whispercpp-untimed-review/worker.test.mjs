...
async function harness({ cancelAtPhase, loadGate, runGate, coreError, closeAcknowledged = true, runOutput } = {}) {
  const events = [], calls = []
    ...
    './worker-protocol.mjs': { createSpeechWorkerProtocol: options => async message => {
      ...
      if (message.kind === 'load') { await loadGate?.promise; response = { kind: 'ready', heapBytes: 67108864 } }
        ...
        response = coreError ? { kind: 'error', ...coreError } : { kind: 'result', heapBytes: 134217728,
          generatedTokens: 12, timing: 'model', segments: [{ text: ' bounded source', fromCentiseconds: 0,
            toCentiseconds: message.pcm.byteLength / 640 }] }
      }
      if (message.kind === 'run' && runOutput) response = { kind: 'result', heapBytes: 134217728,
        generatedTokens: 12, ...runOutput(calls.filter(kind => kind === 'run').length) }
      options.emit({ v: 1, owner: message.owner, id: message.id, ...response })
    } },
  }
  const module = new vm.SourceTextModule(read('scripts/issue201/whispercpp-untimed-review/model-worker.mjs').toString(), {
...
const untimed = { timing: 'unavailable', reason: 'timestamp-coverage', text: ' words require timing' }
const timed = { timing: 'model', segments: [{ text: ' ordinary timed words', fromCentiseconds: 0, toCentiseconds: 100 }] }
test('one untimed window retains all text and source coverage, creates no cues and continues all twelve windows without reload', async () => {
  const h = await harness({ runOutput: index => index === 3 ? untimed : timed })
  await h.initialize(); await h.transcribe()
  const result = h.events.find(event => event.type === 'complete')
  assert.equal(result.windows.length, 12)
  assert.equal(h.calls.filter(kind => kind === 'load').length, 1)
  assert.equal(h.calls.filter(kind => kind === 'run').length, 12)
  const window = result.windows[2]
  assert.equal(window.start, 50); assert.equal(window.end, 80)
  assert.equal(window.timing, 'unavailable'); assert.equal(window.reason, 'timestamp-coverage')
  assert.equal(window.text, untimed.text); assert.equal(window.chunks.length, 0)
  assert.equal('timestamp' in window, false)
  assert.equal(result.windows[3].timing, 'model')
  assert.equal(result.windows[3].chunks[0].timestamp[1], 1)
  assert.equal(result.ledger.maxPcmBytes, 3840000)
  await h.send({ type: 'dispose' })
  assert.equal(h.events.find(event => event.type === 'disposed').cooperativeZero, true)
})
test('short source follows the explicit untimed contract without inventing one-second cue endpoints', async () => {
  const h = await harness({ runOutput: () => untimed })
  await h.initialize(); await h.transcribe(1)
  const window = h.events.find(event => event.type === 'complete').windows[0]
  assert.equal(window.end, 1); assert.equal(window.timing, 'unavailable')
  assert.equal(window.chunks.length, 0); assert.equal('timestamp' in window, false)
  await h.send({ type: 'dispose' })
})
test('untimed text after cancellation is discarded while cleanup remains acknowledged', async () => {
  const runGate = deferred(), h = await harness({ runGate, runOutput: () => untimed })
  await h.initialize(); const pending = h.transcribe()
  await until(() => h.calls.includes('run')); await h.send({ type: 'cancel' })
  runGate.resolve(); await pending; assertDisposed(h)
  assert.deepEqual(h.calls, ['load', 'run', 'close'])
})
test('unknown timing state, malformed untimed text, hidden cue arrays and invalid timed endpoints stay terminal', async () => {
  for (const output of [
    { ...untimed, reason: 'unknown' }, { ...untimed, text: ' ' },
    { ...untimed, text: 'x'.repeat(20001) }, { ...untimed, segments: [] },
    { ...timed, timing: 'unknown' },
    { ...timed, segments: [{ text: ' bad endpoints', fromCentiseconds: 0, toCentiseconds: 3001 }] },
  ]) {
    const h = await harness({ runOutput: () => output })
    await h.initialize(); await h.transcribe()
    assert.equal(h.events.some(event => event.type === 'complete' || event.type === 'window'), false)
    assert.equal(h.events.find(event => event.type === 'error').cooperativeZero, true)
    assert.deepEqual(h.calls, ['load', 'run', 'close'])
  }
})
