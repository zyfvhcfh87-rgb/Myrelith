...
int validate_output(int sample_count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // Overhang forbids timed output for the whole window. Finish validating
        // every segment before exposing its text; malformed output stays fatal.
        if (t0 < previous_end) return -942;
        if (t1 <= t0) return -943;
        if (t1 > sample_count / 160) coverage_failed = true;
    ...
    timing_unavailable = coverage_failed;
    // 1 is a distinct, text-only review result, never successful timed cues.
    return coverage_failed ? 1 : 0;
}
}
...
EMSCRIPTEN_KEEPALIVE uintptr_t speech_pcm_alloc(int count) {
    ...
    ready_segments = 0; generated = 0; timing_unavailable = false;
...
EMSCRIPTEN_KEEPALIVE int speech_run(int language) {
    ...
    ready_segments = 0; generated = 0; timing_unavailable = false; deadline_hit = false;
...
EMSCRIPTEN_KEEPALIVE double speech_segment_t0(int i) {
    return !timing_unavailable && i >= 0 && i < ready_segments ? static_cast<double>(whisper_full_get_segment_t0(context, i)) : -1;
}
EMSCRIPTEN_KEEPALIVE double speech_segment_t1(int i) {
    return !timing_unavailable && i >= 0 && i < ready_segments ? static_cast<double>(whisper_full_get_segment_t1(context, i)) : -1;
}
