...
const profile = path.join(labRoot, 'profile-06')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-bounded-support-runtime',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-06.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-06.jsonl')
...
try {
  ...
  await check('one-second-speech-window', async () => {
    ...
    expect(outcome.state.generation === outcome.beforeGeneration && outcome.state.phase === 'idle'
      ...
      && outcome.state.lastCleanup?.mode === 'terminated-error'
      && outcome.state.lastCleanup.cooperativeZero === false,
    ...
    return { ...outcome, supportLimit: 'Out-of-range model timestamps reject the entire request without cues',
      nativeCleanup: 'acknowledged-zero', workerCleanup: 'terminated-error' }
  })
  ...
  const output = path.join(labRoot, 'runtime-results-06.json')
