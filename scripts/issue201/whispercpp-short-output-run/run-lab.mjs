...
const profile = path.join(labRoot, 'profile-05')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-short-output-runtime',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-05.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-05.jsonl')
...
try {
  ...
  const output = path.join(labRoot, 'runtime-results-05.json')
