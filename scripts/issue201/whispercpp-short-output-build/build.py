"""Reuse the reviewed builder with the isolated short-output diagnostic recipe.
...
EVIDENCE = ROOT / 'docs/evidence/issue201/whispercpp-short-output-source'
ATTEMPT = ROOT / '.tmp/issue201-whispercpp-short-output-build'
...
def prepare_source():
    ...
    for name in ['CMakeLists.txt', 'adapter.cpp', 'myrelith-token-budget.h']:
        data = (ROOT / 'scripts/issue201/whispercpp-short-output-recipe' / name).read_bytes()
...
def verify_prepared():
    ...
    for row in receipt['recipe']:
        ...
        if data != (ROOT / 'scripts/issue201/whispercpp-short-output-recipe' / row['path']).read_bytes():
