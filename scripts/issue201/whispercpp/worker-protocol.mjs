...
export function readSegments(module, samples) {
  ...
  if (!Number.isInteger(count) || count < 0 || count > 1000) fail('segment-count');
  const segments = []; let previousEnd = 0, characters = 0;
  for (let i = 0; i < count; i++) {
    const from = module._speech_segment_t0(i), to = module._speech_segment_t1(i);
    if (!Number.isSafeInteger(from) || !Number.isSafeInteger(to) || from < previousEnd
    ...
    segments.push({ text, fromCentiseconds: from, toCentiseconds: to }); previousEnd = to;
  }
  return segments;
}
  ...
  return async function receive(message) {
    ...
    try {
      ...
      if (message.kind === 'load') {
        ...
        if (status !== 0) fail(`inference-${status}`);
        ...
        const segments = readSegments(module, samples);
        state = 'ready'; emit({ v: 1, owner, id, kind: 'result', generatedTokens, segments, heapBytes: heap().byteLength });
