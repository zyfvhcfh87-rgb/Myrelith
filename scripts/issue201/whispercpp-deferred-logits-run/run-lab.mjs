...
const profile = path.join(labRoot, 'profile-04')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-deferred-logits-runtime',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-04.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-04.jsonl')
...
try {
  ...
  const output = path.join(labRoot, 'runtime-results-04.json')
