...
const profile = path.join(labRoot, 'profile-07')
...
if (git('status', '--porcelain', '--', 'scripts/issue201', 'docs/evidence/issue201/whispercpp-cancellation-runtime.json',
...
await bounded(writeFile(path.join(labRoot, 'runtime-attempt-07.json'), JSON.stringify({ pid: process.pid,
...
const journalPath = path.join(labRoot, 'runtime-partial-07.jsonl')
...
try {
  ...
  await check('one-second-speech-window', async () => {
    // The accepted support limit rejects this fixture's out-of-range model
    // endpoint. Preserve runs04/05, which failed the former success expectation.
    const outcome = await page.evaluate(async () => {
      const before = lab.state()
      let result = null, error = null
      try { result = await lab.transcribeFixture('english', { seconds: 1 }) }
      catch (cause) { error = { message: cause.message, code: cause.code, phase: cause.phase } }
      const state = lab.state()
      return { beforeGeneration: before.generation, result, error, state,
        events: state.events.slice(before.events.length) }
    })
    expect(outcome.error?.message === 'Core inference--944; cooperativeZero=true'
      && outcome.error.code === 'transcription-failed' && outcome.error.phase === 'infer',
    'Short source did not reject with the exact acknowledged native coverage error')
    expect(outcome.result === null && !outcome.events.some(event => ['window', 'complete'].includes(event.type)),
      'Rejected short source returned transcript cues')
    const ledger = outcome.events.find(event => event.type === 'error')?.ledger
    expect(ledger?.inputOwners === 0 && ledger.sampleOwners === 0 && ledger.pcmBytes === 0
      && ledger.acquiredSamples === ledger.closedSamples, 'Rejected short source retained audio owners')
    expect(outcome.state.generation === outcome.beforeGeneration && outcome.state.phase === 'idle'
      && outcome.state.workerOwners === 0 && outcome.state.acquisitionOwners === 0
      && outcome.state.lastCleanup?.mode === 'cooperative-error'
      && outcome.state.lastCleanup.cooperativeZero === true,
    'Short-source rejection changed project generation or failed to release its worker')
    return { ...outcome, supportLimit: 'Out-of-range model timestamps reject the entire request without cues',
      nativeCleanup: 'acknowledged-zero', workerCleanup: 'cooperative-error' }
  })
  ...
  const output = path.join(labRoot, 'runtime-results-07.json')
