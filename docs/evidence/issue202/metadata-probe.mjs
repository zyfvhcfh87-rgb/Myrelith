...
const artifactPrefix=path.basename(outputPath,'.json');
...
for(const format of ['mp4','webm']) for(const cell of cells){
  if(fs.existsSync(path.join(root,`${artifactPrefix}-${format}-${cell.id}.container.bin`))) throw new Error('Refusing to overwrite structural evidence');
}
const rows=[];
for(const format of ['mp4','webm']) for(const cell of cells){
  ...
  try{
    ...
    const filename=`${artifactPrefix}-${format}-${cell.id}.container.bin`;
    fs.writeFileSync(path.join(root,filename),bytes,{flag:'wx'});
