...
export async function runR3() {
  ...
  async function runJob(job) {
    ...
    let worker;
    try { worker = new Worker(new URL('./r3-worker.mjs', import.meta.url), {type: 'module'}); }
  ...
  if (!result.numericPassed) {
    ...
    for (const [width, height] of [[1920, 1080], [3840, 2160]]) {
      for (const phase of ['preview', 'export']) for (let repetition = 0; repetition < REPETITIONS; repetition++) {
        if (abort) break;
