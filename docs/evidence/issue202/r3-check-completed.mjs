...
delete env.ISSUE202_R3_COMPLETION_SLOT;
...
const tests = execFileSync(process.execPath, ['--test', path.join(root, 'r3-ledger.test.mjs'), path.join(root, 'r3-decisions.test.mjs'), path.join(root, 'r3-completion.test.mjs')], {env, encoding: 'utf8'});
const reference = execFileSync('python3', [path.join(root, 'r3-make-reference.py'), '--verify'], {env, encoding: 'utf8'});
const gate = spawnSync(process.execPath, [path.join(root, 'run-r3-completed.mjs'), path.join(root, 'r3-not-authorized.json')], {env, encoding: 'utf8'});
assert.equal(gate.status, 1); assert.match(gate.stderr, /Fresh exclusive R3 completion-correction slot required/);
...
const built = await build({configFile: false, root: repository, publicDir: false, plugins: [audit()],
  ...
  build: {write: false, minify: false, lib: {entry: path.join(root, 'r3-controller-completed.mjs'), formats: ['es']}}});
...
const result = {kind: 'issue202-r3-completion-preparation-only', browserLaunched: false, shaderExecuted: false,
...
process.stdout.write(JSON.stringify({output, staticTests: 14, referenceCases: 40, productionModulesCompiled: sourceFiles.length,
