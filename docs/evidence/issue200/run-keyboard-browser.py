...
def main():
    ...
    artifacts = Path(os.environ['ISSUE200_KEYBOARD_ARTIFACTS'])
    manifest = os.environ['ISSUE200_KEYBOARD_MANIFEST']
    ...
    source_check = ['node', 'docs/evidence/issue200/verify-keyboard-source.mjs', 'verify', manifest]
    ...
    command = ['node', 'node_modules/@playwright/test/cli.js', 'test', '--config', 'tests/diagnostics/issue200/keyboard.playwright.config.ts']
    owned = {}
    ...
    try:
        ...
        if not stdout_complete:
            ...
            try:
                ...
                timed_out = True
        if timed_out:
            observer_errors.append('The 600-second keyboard run deadline expired; stdout may be incomplete')
            code = 1
