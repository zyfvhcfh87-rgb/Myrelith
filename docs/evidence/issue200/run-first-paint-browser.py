...
def main():
    ...
    artifacts = Path(os.environ['ISSUE200_FIRST_PAINT_ARTIFACTS'])
    manifest = os.environ['ISSUE200_FIRST_PAINT_MANIFEST']
    ...
    source_check = ['node', 'docs/evidence/issue200/verify-first-paint-source.mjs', 'verify', manifest]
    ...
    command = ['node', 'node_modules/@playwright/test/cli.js', 'test', '--config', 'tests/diagnostics/issue200/first-paint.playwright.config.ts']
    owned = {}
    ...
    try:
        ...
        if not stdout_complete:
            ...
            try:
                ...
                timed_out = True
        if timed_out:
            observer_errors.append('The 600-second first-paint run deadline expired; stdout may be incomplete')
            code = 1
...
def write_artifact_manifest():
    """After Playwright exits, include success/failure trace ZIPs and all files."""
    import hashlib
    artifacts = Path(os.environ['ISSUE200_FIRST_PAINT_ARTIFACTS'])
    ...
    for path in sorted(artifacts.rglob('*')):
        if path.is_symlink():
            raise RuntimeError('Evidence must not contain a symlink')
        if not path.is_file() or path.name == 'artifact-manifest.json':
            continue
        payload = path.read_bytes()
        members.append({'path': str(path.relative_to(artifacts)), 'bytes': len(payload),
                        'sha256': hashlib.sha256(payload).hexdigest()})
    artifacts.joinpath('artifact-manifest.json').write_text(json.dumps({'version': 1,
        'files': members, 'tracePolicy': 'on; success and failure retained'}, indent=2) + '\n')
