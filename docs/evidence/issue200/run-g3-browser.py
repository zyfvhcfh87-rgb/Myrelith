...
code = 1
def expire():
    observer_errors.append('The 600-second G3 run deadline expired')
    if child.poll() is None:
        try: os.killpg(child.pid, signal.SIGTERM)
        except ProcessLookupError: pass
deadline = threading.Timer(600, expire)
...
try:
    ...
    time.sleep(0.2)
    artifacts.joinpath('process-ownership.json').write_text(json.dumps({'launcherPid': child.pid, 'exitCode': code, 'observed': list(owned.values()), 'requiredTermination': survivors, 'remaining': remaining(), 'observerErrors': observer_errors}, indent=2))
try:
    ...
    if release['remainingOwned'] or listener.returncode != 1 or listener.stdout or observer_errors:
        code = 1
