...
| Proxy video encode | MP4 + AVC, runtime-probed with `Mp4OutputFormat.getSupportedVideoCodecs()` plus a disposable one-frame `CanvasSource` using the exact output width, height, bitrate, key-frame interval, and source rational frame rate before output allocation | Codec/container substitution, software/WASM encoder fallback, omitting the actual frame rate from the probe, or starting generation when the exact native AVC configuration is unavailable |
...
configuration, including the source rational frame rate passed to the encoder.
The exact probe initializes the same opaque sRGB conversion canvas as generation
and muxes only to a null target, so no OPFS bytes are acquired or written.
Unsupported input/output stays disabled with its concrete reason. Generation
repeats both checks before acquiring its one-shot OPFS file capability, so a
stale capability result cannot create committed bytes.
...
abort staged bytes, and publish a retryable state. Generation covers only the
durable primary-video span (`endTimestamp - firstTimestamp`), samples within
that exact extent even when the first PTS is negative/nonzero, and normalizes
proxy timestamps to zero; a longer audio track never extends proxy video. A
manifest entry is written only after mux finalization and OPFS commit.
...
Manifest parsing rejects unknown fields and bounds every identifier, filename,
dimension, bitrate, duration, timestamp, byte count, and reduced rational frame
rate before values can enter cache arithmetic. Cache replacement is a two-phase
transaction: it commits the new manifest, then either finalizes old-file cleanup
after the controller's final source/lifecycle guard or rolls back the manifest
and late output. Remove and clear invalidate the generation, await scheduler
idle, and serialize through the same mutation tail, so a queued commit cannot
resurrect state or bytes. Capacity stays below both 80% of the browser-reported origin quota and
