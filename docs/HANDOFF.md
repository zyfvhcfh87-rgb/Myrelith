...
| **Post-MVP #191 — multiple project sequences** | 🚧 implementation complete locally; delivery authorized | project-format-6 same-settings sequence collection; explicit portable root/session active; whole-project history, CRUD UI, validation, persistence/recovery/media reconciliation, and explicit export target; post-rebase 227 focused + 3,726 total Vitest and 17 runner tests; in-app Chromium v5 migration/CRUD/root/undo/redo/export/recovery/720px gate clean; exact-head Bugbot/CI review pending |
